...
2. ~~**P2 90-day gate walk on PUBLISHED npm**~~ **DONE — 5/5 PASS, CC2 wakeup 103 2026-09-13.** Clean temp dir + `npm i @hyperdag/trustshell@1.3.0` (registry current 1.3.0). Keyless vs prod: init/health ok=true; verifyOutput Paris→PASS, Rome→VETO; getRepID(trinity-shofet)→repid 2177/ESTABLISHED; presentProof({verify:true})→{agentId,tier,proofBytes,scheme,statement,createdAt}. No mint (Bearer-gated, out of scope). Receipt: `docs/living/NPM_GATE_RECEIPT.md`.
3. ~~**P3 429-dedup**~~ **DONE — documented + guarded, CC2 wakeup 104 2026-09-13 (DRAFT PR #153).** Live re-probe: 3 identical back-to-back POST /register → 201/201/201, distinct ids. Root cause: repid-engine dedup is a per-PROCESS in-memory Map (its own comment: "move to Redis") → dormant on multi-replica Railway, so the 429 branch never fires; live = 201-every-time = fresh agent, never reuse, no 409. Fixed the DOC (CREATE_PAI_UI.md caveat (b) → live reality) + added non-flaky parser test (two 201s = distinct agentIds). Making 429 reliable (Redis) = **backend/infra call for Sean**. Did NOT implement global uniqueness or a 409 (Sean's call).
4. ~~**P4 stretch — ingest quarantine harden**~~ **DONE — CC2 wakeup 105 2026-09-13 (DRAFT PR #154).** (a) bypassable blocklist → detection now on raw + de-obfuscated (zero-width/joiner strip + letter-spacing collapse) + base64-decoded views, plus paraphrase/exfil patterns; (b) flag tier honest → CLEAN is the only "proceed" verdict, veto/flag are holds. 4 new fixtures + tests/ingest.test.ts 12/12; verify green 384. Kept default-off/unexported/unwired (verified). INGEST_EVAL.md updated. Known ceiling unchanged (model classifier is the real fix).

---
## ✅ CC2 QUEUE EMPTY — STOPPED (2026-09-13 17:xx PDT), will resume on new dispatch.
All 4 NOW items done as DRAFT PRs (none merged, no publish): P1 #58 live-stats = NOT REPRODUCIBLE/stale premise (PR #152 guard) · P2 npm@1.3.0 gate walk 5/5 PASS (receipt) · P3 429-dedup drift documented+guarded (PR #153) · P4 ingest-harden (PR #154). Cron stopped; not rescheduling. Open for Sean: (i) taken-PAI-name 409 vs non-unique labels; (ii) move repid-engine dedup to Redis (in-memory per-process → dormant on multi-replica); (iii) stuck live-stats widget (if ever) = stale deploy, not a key. Merges + publish + key rotation remain Sean-only.
